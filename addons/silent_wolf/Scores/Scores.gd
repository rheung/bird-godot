...
func get_scores(maximum: int=10, ldboard_name: String="main", period_offset: int=0) -> Node:
	...
	GetScores.request_completed.connect(_on_GetScores_request_completed.bind(GetScores, wrGetScores))
...
func _on_GetScores_request_completed(result, response_code, headers, body, request_node: HTTPRequest, request_weakref) -> void:
	var status_check = SWUtils.check_http_response(response_code, headers, body)
	SilentWolf.free_request(request_weakref, request_node)
